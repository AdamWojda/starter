...
"use strict";!function(e){function t(e){return e.replace(/\b((http|https):\/\/\S+)/g,'<a href="$1" target="_blank">$1</a>')}function a(e){var t=new Date(e),a=new Date-t;if(a<=1e4)return"Just now";var i=null,s={millisecond:1,second:1e3,minute:60,hour:60,day:24,month:30,year:12};for(var r in s){if(a<s[r])break;i=r,a/=s[r]}return a=Math.floor(a),1!==a&&(i+="s"),[a,i,"ago"].join(" ")}e.fn.facebookWall=function(s){if(s=s||{},!s.id)throw new Error("You need to provide an user/page id!");if(!s.access_token)throw new Error("You need to provide an access token!");s=e.extend({limit:25},s);var r="https://graph.facebook.com/"+s.id+"/?fields=name,picture&access_token="+s.access_token+"&callback=?",c="https://graph.facebook.com/"+s.id+"/posts/?access_token="+s.access_token+"&callback=?&date_format=U&limit="+s.limit,n=this;return e.when(e.getJSON(r),e.getJSON(c)).done(function(s,r){var c={user:s[0],posts:[]};e.each(r[0].data,function(){return"link"!=this.type&&"status"!=this.type||!this.message||(this.from.picture=c.user.picture.data.url,this.created_time=a(1e3*this.created_time),this.message=t(this.message),void c.posts.push(this))});var o=c.posts;for(i=0;i<o.length;i++){var d,_=(o[i].id,o[i].from.name),l=o[i].from.id,f=o[i].from.picture,h=o[i].created_time,p=o[i].message,m=o[i].name,u=o[i].link,v=o[i].caption,k=o[i].description,g=o[i].picture,w=o[i].type;void 0==o[i].shares&&(d="0"),void 0!==o[i].shares&&(d=o[i].shares.count);var b,y='<div class="feed__element"> <img class="feed__page-avatar" src="'+f+'" alt="'+_+'" />',S='<div class="feed__status"><h2 class="feed__heading"><a class="feed__heading__link" href="http://www.facebook.com/profile.php?id='+l+'" target="_blank">'+_+"</a></h2>",j='<p class="feed__message">'+p+"</p>",J='<div class="feed__attachment">',O='<img class="feed__attachment__picture" src="'+g+'" />',$='<div class="feed__attachment__content"><p class="feed__attachment__name"><a href="'+u+'" target="_blank">'+m+'</a></p><p class="feed__attachment__caption">'+v+'</p><p class="feed__attachment__description">'+k+"</p></div>",A="</div>",D="</div>",E='<div class="feed__meta"><div class="feed__time">'+h+'</div><div class="feed__shares">'+d+"</div></div>",N="</div>";"link"==w&&(b=J+O+$+A);var Q=y+S+j+b+D+E+N;e(n).append(Q)}}),this}}(jQuery),$("#wall").facebookWall({id:"jqueryscript",access_token:"193673707489176|CSya6L7Q081Uw6OrFfI14AsmfSA"});